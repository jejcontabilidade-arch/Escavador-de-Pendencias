...
### Incidente/Melhoria 4: Resolução 100% Autônoma de Captcha por IA (Sem Fallback Manual)
* **Problema:** A automação exigia a intervenção manual do usuário no WhatsApp ou no navegador para resolver captchas de imagens do hCaptcha/reCAPTCHA, impossibilitando a execução de varreduras gerais noturnas ou autônomas sem a presença física de um operador humano.
* **Causa Raiz:** O robô de Notas Fiscais XML não possuía inteligência visual integrada para classificar e responder aos desafios de imagens diretamente. O sistema enviava um print da tela para o WhatsApp e aguardava indefinidamente a criação de um arquivo `captcha_response.json` com os cliques do operador humano.
* **Solução Definitiva:**
  1. **Integração de IA Visual (GPT-4o Vision):** Implementada a função `resolver_captcha_com_openai` em `consultar_nota_fiscal_xml.py` que converte a imagem do Captcha em Base64 e a envia à API da OpenAI usando o modelo `gpt-4o`. O prompt instrui a IA a analisar o desafio (como *"Selecione todas as imagens contendo barcos"*) e mapear os cliques linha por linha da esquerda para a direita no grid de imagens (3x3 ou 4x4).
  2. **Interação Autônoma com o DOM:** O robô extrai o texto da pergunta da tela e tira um screenshot focado especificamente no frame de desafio do Captcha para otimizar os tokens e aumentar a precisão de análise. Ele simula os cliques nas coordenadas do grid fornecidas pelo GPT-4o Vision e aciona o botão de verificação.
  3. **Zero Intervenção Manual:** Eliminadas todas as rotinas que notificavam o WhatsApp ou aguardavam o arquivo de resposta do operador. O robô tenta resolver de forma automática por IA por até 5 vezes em loop (para contornar captchas em cascata). Caso atinja o limite sem sucesso, registra a falha do cliente atual no log/status e passa silenciosamente para a próxima empresa da fila.
