...
Procima tarefa: Para obter o token de acesso à API de DF-e (FocusNFe), você deve seguir os seguintes passos:

Criar uma conta no FocusNFe: Acesse o site oficial da FocusNFe e realize o cadastro da sua empresa. Eles oferecem um serviço de integração fiscal e emissão/consulta de documentos para desenvolvedores e contabilidades.

Obter a Chave no Painel: Após criar a conta e acessar o painel de controle da FocusNFe, você encontrará o seu Token de API nas configurações de integração (geralmente sob a aba "API" ou "Minha Conta"). Eles fornecem dois tipos de tokens:

Token de Homologação: Para realizar testes e simulações gratuitas.
Token de Produção: Para consultas de notas fiscais reais em ambiente oficial da Receita.
Configurar no Escavador: Insira o token obtido no seu arquivo de configuração config.json ou config_private.json na raiz da pasta do projeto, da seguinte forma:

json
{
  "focusnfe_token": "SEU_TOKEN_AQUI",
  "focusnfe_env": "producao" 
}
(Você pode mudar "focusnfe_env" para "homologacao" se estiver usando o ambiente de testes da FocusNFe).

💡 Caso você não queira assinar a API agora:
O sistema foi desenvolvido de forma resiliente. Se a chave "focusnfe_token" estiver ausente ou vazia nas configurações, o robô irá simplesmente pular a requisição de API e acionar de forma 100% automática a contingência baseada em navegador (Playwright), buscando os XMLs de forma gratuita diretamente no site nacional da NF-e com os certificados locais instalados.
